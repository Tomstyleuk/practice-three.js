...
function init() {
    ...
    const camera = new THREE.PerspectiveCamera(3, width / height );
    camera.position.set(0, 0, +1000);

    //5.立方体を作る. 立方体はメッシュという表示オブジェクトを使用して作成します。
    //メッシュを作るには、ジオメトリ（形状）とマテリアル（素材）の二種類を用意する必要があります
    //立方体や直方体のような箱状の形状を生成するためのBoxGeometryを使用
    const geometry = new THREE.CylinderGeometry(5, 20, 30, 30);  //(width, height, depth)
    ...
    function tick() {
        ...
        box.rotation.y += 0.01; //横回転 数値が高いほど、回転する速度が上がる
        box.rotation.x += 0.01;　//縦回転
